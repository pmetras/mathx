...
actor Main
  ...
  fun pi(env: Env) =>
    ...
    Translation of the mathematical formulais is almost immediate. From a
  ...
  fun _string(x: SMPFr): String iso^ =>
    """
    Print the result as the `MPF` does not have an output function yet.
    """
    let result: String iso = recover iso
      var cnt: I32 = @mpfr_snprintf(Pointer[U8], 0, "%.RNe".cstring(), x)
      var buffer: Pointer[U8] = @pony_alloc(@pony_ctx(), cnt.usize())

      cnt = @mpfr_snprintf(buffer, cnt.u64(), "%.RNe".cstring(), x)
